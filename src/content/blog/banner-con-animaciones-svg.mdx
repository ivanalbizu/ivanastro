...
slug: banner-con-animaciones-svg
published: true
...
categories:
  ...
  - css
  - svg
date: "2019-11-17 15:10:00"
image: ../../assets/images/banner-con-animaciones-svg.jpg
image_alt: Banner con animaciones SVG
---
...
<Codepen id="zYYyrRJ" defaultTab="result" name={frontmatter.title} />
...
* <code>const coord = el => {}</code>.
* <code>const handleMove = event => {}</code>.
* <code>const handleClick = event => {}</code>.
* <code>const openBox = (box) => {}</code>.
* <code>const closeBox = (box) => {}</code>.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (/Edge\/\d./i.test(navigator.userAgent)){
    ...
    boxes.forEach(box => {
      box.style = `
        background-image: ../../assets/images/url(${box.querySelector('image').getAttribute('xlink:href')});
        background-size: 100%;
