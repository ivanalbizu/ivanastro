---
title: "Web Component de Partículas con Canvas: Animaciones Nativas"
slug: webcomponent-canvas-particles
description: "Aprende a crear un Web Component nativo que renderiza partículas interactivas usando HTML5 Canvas. Tutorial paso a paso para encapsular animaciones de alto rendimiento con JavaScript."
published: true
categories:
  - javascript
  - html
  ...
  - web-components
  - canvas
date: '2022-01-19 17:40:00'
image: ../../assets/images/webcomponent-canvas-particles.png
image_alt: Componente Web con Partículas en lienzo Canvas
---

Las animaciones con partículas son un recurso visual potente para dar vida a una interfaz. En este artículo, combinaremos la potencia del **HTML5 Canvas API** con la encapsulación de los **Web Components** para crear un elemento reutilizable `<canvas-draw>` que puedes integrar en cualquier proyecto web sin depender de frameworks externos.

Antes de entrar en código, quiero recomendar el canal de YouTube de Frank's Laboratory. Contiene excelentes tutoriales sobre Canvas con JavaScript nativo, explicados desde cero y aumentando en complejidad. Es una fuente de inspiración genial para este tipo de efectos.

import Codepen from '../../components/Codepen.astro'

<Codepen id="LYzvxqz" defaultTab="result" name={frontmatter.title} />

## 1. Configuración del Entorno (Opcional)

Para desarrollar este componente de forma aislada, utilizaremos **ParcelJS** como empaquetador rápido, aunque el código final del componente es JavaScript puro (Vanilla) y funciona en cualquier entorno.

Si quieres seguir el ejemplo localmente:
...
* Su posicionamiento mediante <code>x</code> e <code>y</code>
* Su color de relleno
...
* <code>size</code>: tamaño inicial de la partícula
* <code>speedX</code> y <code>speedY</code>: dirección del movimiento(*)
...
* <code>update()</code>: actualiza las propiedades de las partículas
* <code>draw()</code>: pinta las partículas
...
* <code>static get observedAttributes()</code>: incluimos en el array aquellos atributos que pudieran ser modificados
* <code>attributeChangedCallback(name, oldValue, newValue)</code>: asignamos al atributo su nuevo valor
...
* ><code>event.x</code> y <code>event.y</code>: obtenemos las coordenadas x e y asociadas al evento
* ><code>cssObj.getPropertyValue("background-color")</code>: obtenemos el color de fondo del elemento <code>js-particles</code> sobre el que se hizo click
...
<Codepen id="LYzvxqz" defaultTab="js" name={frontmatter.title} />
